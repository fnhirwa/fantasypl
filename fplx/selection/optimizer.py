...
class SquadOptimizer(BaseOptimizer):
    ...
    def validate_squad(self, players: list[Player]) -> bool:
        ...
        """
        return (
            self.formation_constraint.validate(players) and
            self.budget_constraint.validate(players) and
            self.diversity_constraint.validate(players)
        )
    ...
    def optimize(
        ...
        for pos in ['GK', 'DEF', 'MID', 'FWD']:
            ...
            for candidate in candidates:
                ...
                if self.diversity_constraint.validate(temp_squad) and self.budget_constraint.validate(temp_squad):
        ...
        if len(selected) < 11:
            logger.warning(f"Greedy selection only picked {len(selected)} players. Filling with cheapest options.")
        ...
        if len(selected) != 11:
            logger.error(f"Optimizer could not form a valid 11-player squad. Only found {len(selected)}.")
...
class ILPOptimizer(SquadOptimizer):
    ...
    def optimize(
        ...
        # Decision variables
        player_vars = {
            p.id: self.pulp.LpVariable(f"player_{p.id}", cat='Binary')
            for p in players
        }
        
        # Objective: maximize expected points
        prob += self.pulp.lpSum([
            expected_points.get(p.id, 0) * player_vars[p.id]
            for p in players
        ])
